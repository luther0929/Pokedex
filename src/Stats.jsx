...
export default function Stats({pokemon}){
    ...
    return(
        <animated.div style={fade} className="grid h-40 pl-3 m-4 text-base text-left text-gray-800 lg:gap-2 lg:h-50">
            {pokemon.stats.map((stat)=> {
                const barWidth = getStatBarWidth(stat.base_stat)
                const changedName = getShortenedName(stat.stat.name)
                return (
                    <div className='grid items-center grid-cols-10' key={stat.stat.name}>
                        <p className='col-span-2 text-gray-400 lg:col-span-1'>{changedName}</p>
                        <p className="col-span-2 text-center">{stat.base_stat}</p>
