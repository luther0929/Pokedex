...
export function useFetchPokemonData(limit=151, maxRetries=3){
    ...
    useEffect(() => {
        ...
        setState({ //resets the state values
            data: [],
            isLoading: true,
            error: null
        });
        ...
        const attemptFetch = async() => {
            try{
                ...
                const pokemonsResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=${limit}`, { signal }); //attach signal to cancel fetching
                if (!pokemonsResponse.ok){
                    throw new Error(`Unable to fetch pokemon list ${pokemonsResponse.status}`)
                }
                const pokemonsData = await pokemonsResponse.json();

                if(!pokemonsData?.results?.length){ //safely checks if pokemons.result is null
                    setState({
                        data: [],
                        isLoading: false,
                        error: null
                ...
                const pokemonData = await Promise.all(pokemonsData.results.map(async(pokemon) => { //fetches each pokemon's data
                    ...
                    const specieResponse = await fetch(`https://pokeapi.co/api/v2/pokemon-species/${pokemon.name}/`, {signal});
                    if (!specieResponse.ok){
                        throw new Error(`Failed to fetch pokemon specie data: ${specieResponse.status}`);
                    }
                    ...
                    return { //return the only needed basic data
                        id: basicData.id,
                        name: basicData.name,
                        height: basicData.height / 10, //convert to meters
                        weight: basicData.weight / 10, //convert to kilograms
                        abilities: basicData.abilities.map((ability) => ability.ability.name),
            ...
            }catch(error){ //update error value if there is one
