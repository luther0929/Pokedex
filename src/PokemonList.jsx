    ...
    return(
        <div className="flex flex-row w-screen pt-2 overflow-y-scroll text-gray-800 h-30 lg:w-1/3 lg:h-screen lg:flex-col ">
            {pokemons.map((pokemon) => {
                ...
                return(
                    <div className="flex justify-between m-2 rounded-lg border-1 drop-shadow-xl"key={pokemon.name} onClick={() => onPokemonClick(pokemon.id)} style={{backgroundColor: lightenColor(pokemon.color)}}>
                        <div className='w-30 lg:w-1/3'>
                            <div className='justify-between m-2 lg:flex-row'>
                                <h1 className='text-xl font-bold text-white opacity-60 lg:text-2xl [text-shadow:_2px_2px_2px_rgb(0_0_0)]'>{adjustId(pokemon.id)}</h1>
